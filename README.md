...
2. Open the [pre-filled Nightwatch GitHub Issues token form](https://github.com/settings/personal-access-tokens/new?name=Nightwatch+production+issues&description=Create+and+synchronize+GitHub+Issues+from+Laravel+Nightwatch&expires_in=90&issues=write). It requests a 90-day token with only `Issues: write`; review every field before generating it. Alternatively, open [GitHub fine-grained personal access tokens](https://github.com/settings/personal-access-tokens), select **Generate new token**, and configure it manually.
3. Confirm that the token type is fine-grained, not classic.
...
The pre-filled link never contains or receives a token. It only opens GitHub's own form with suggested values; the credential is generated by GitHub after you review and submit that form.
